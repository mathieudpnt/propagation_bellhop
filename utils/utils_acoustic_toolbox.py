...
def read_arrivals_asc(filename: Path) -> ndarray[tuple[Any, ...]]:
    ...
    with Path.open(filename, "rb") as fid:
        ...
        source_depth = readline_1(fid, 1)  # source depth
        receiver_depth = readline_1(fid, 1)  # receiver depth
        receiver_range = readline_1(fid, 1)  # receiver range

        # Initiation of the components of arr
        narr = int(fid.readline())  # total number of studied rays
        wave_eq = zeros(narr, 1)  # wave equation
        ...
        for k in range(narr):
            (amp, phase, rtau, itau,
             ...
             num_top_bnc[k], num_bot_bnc[k]) = (fid.readline().split())
